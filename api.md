...
As the addon allows using multiple keys, the authentication method also uses multiple keys. This is done by providing
`auth_keys` as a parameter with a comma-separated list of key hashes.
...
The following example assume that two keys were used in `auth_keys`:
...
The `keys` section contains the state of keys included in `auth_keys`. The `friends` section includes subtokens for
clears shared with any of the keys included in `auth_keys`.
...
This is an example that includes some subtokens for friends that were shared with one of our auth_keys.
...
- `auth_keys` – comma-separated list of key hashes

### POST /key/add_subtoken

This endpoint adds a subtoken to a key

#### Params

- `auth_keys` – comma-separated list of key hashes
- `key_hash` – key hash
- `subtoken` – see requirements below

#### Subtoken requirements

The subtoken expiration date **has to be set to year 2100 or more.**
